...
TRACKER_CSV = "csv_management/csv_files/usernames_tracker.csv"
            ...
            try:
                ...
                if follow_btn.is_visible(timeout=4000):
                    ...
                    if btn_text in ("Following", "Requested"):
                        print(f"[{username}]    ⏭️ Already following @{target}")
                        continue
                    follow_btn.click()
                    human_delay(2, 4)
                    ...
                    print(f"[{username}]    ⚠️ No Follow button for @{target}")

            except Exception as e:
                print(f"[{username}]    ❌ Error on @{target}: {e}")
                errors += 1

            # Wait between follows (shorter for test run of 10)
            if i < len(targets) - 1:
...
def run_parallel_follows(num_accounts=2, follows_per_account=10):
    ...
    # Prefer accounts with valid cookies first
    import os
    accounts_sorted = sorted(
        accounts,
        key=lambda a: os.path.getsize(f"sessions/{a['username']}_state.json")
                      if os.path.exists(f"sessions/{a['username']}_state.json") else 0,
        reverse=True
    )
    ...
    # Split targets between accounts
    account_targets = {}
    ...
    for t in threads:
        t.start()
        time.sleep(2)  # slight stagger so windows don't open on top of each other

    # Wait for all to complete
    for t in threads:
        t.join()

    # Mark followed targets as used in tracker
    for account in selected:
